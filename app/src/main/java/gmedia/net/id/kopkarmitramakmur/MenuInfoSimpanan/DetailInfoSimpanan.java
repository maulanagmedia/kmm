...
public class DetailInfoSimpanan extends AppCompatActivity {
    ...
    private TextView tvTotalWajib;
    private TextView tvTotalKesetiaan;
    private TextView tvTotalPokok;
    private TextView tvTotalSukarela;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_detail_info_anggota);
    ...
    private void initUI() {
        ...
        setTitle("Info Simpanan");
        spBulan = (Spinner) findViewById(R.id.sp_bulan);
        spTahun = (Spinner) findViewById(R.id.sp_tahun);
        tvNama = (TextView) findViewById(R.id.tv_nama);
        tvTotalWajib = (TextView) findViewById(R.id.tv_total_wajib);
        tvTotalKesetiaan = (TextView) findViewById(R.id.tv_total_kesetiaan);
        tvTotalPokok = (TextView) findViewById(R.id.tv_total_pokok);
        tvTotalSukarela = (TextView) findViewById(R.id.tv_total_sukarela);

        session = new SessionManager(DetailInfoSimpanan.this);
        nama = session.getNama();
        nik = session.getNik();
        tvNama.setText(nama+" (" + nik + ")");
    ...
    private void setListData(){
        ...
        ApiVolley request = new ApiVolley(DetailInfoSimpanan.this, jBody, "GET", WebServiceURL.getSimpanan + nik, "", "", 0, new ApiVolley.VolleyCallback() {
            @Override
                ...
                try {
                    ...
                    if(iv.parseNullInteger(status) == 200){
                        ...
                        for(int i = 0; i < jsonArray.length();i++){
                            ...
                            masterList.add(new CustomListItem(item.getString("id"), Converter.convertBulan(item.getString("bulan")), item.getString("tahun"), item.getString("tot_wajib"), item.getString("tot_kesetiaan"), item.getString("tot_pokok"), item.getString("tot_sukarela")));
                        }
                    }
        ...
        spBulan.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(spBulan.getAdapter() != null && spTahun != null){
                    ...
                    tvTotalWajib.setText(iv.ChangeToRupiahFormat("0"));
                    tvTotalKesetiaan.setText(iv.ChangeToRupiahFormat("0"));
                    tvTotalPokok.setText(iv.ChangeToRupiahFormat("0"));
                    tvTotalSukarela.setText(iv.ChangeToRupiahFormat("0"));
                    ...
                    if(listItem != null && listItem.size() > 0){
                        ...
                        for(CustomListItem item: masterList){
                            ...
                            if(item.getItem2().toLowerCase().equals(selectedBulan.toLowerCase()) && item.getItem3().equals(selectedTahun)){
                                ...
                                tvTotalWajib.setText(iv.ChangeToRupiahFormat(item.getItem4()));
                                tvTotalKesetiaan.setText(iv.ChangeToRupiahFormat(item.getItem6()));
                                tvTotalPokok.setText(iv.ChangeToRupiahFormat(item.getItem7()));
                                tvTotalSukarela.setText(iv.ChangeToRupiahFormat(item.getItem8()));
                            }
                        }
    ...
    private void setBulanAdapter(List<String> listItem){
        ...
        if(listItem != null && listItem.size() > 0){
            ...
            ArrayAdapter adapter = new ArrayAdapter(DetailInfoSimpanan.this, R.layout.normal_spinner, listItem);
